...
// modified version of https://github.com/nasa8x/rss-to-json/blob/master/src/parse.ts
export default async (data: string) => {
  ...
  const rss = {
    ...
    image: channel.image
      ...
      : channel['itunes:image']
        ...
        : '',
    category: channel.category || channel['itunes:category'] || [],
    items: []
  }
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const obj = {
      ...
      guid: val.guid && val.guid.$text ? val.guid.$text : val.guid,
      description:
